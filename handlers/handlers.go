...
import (
	...
	"github.com/FrancoRutigliano/gambit/auth"
	"github.com/FrancoRutigliano/gambit/routers"
...
func validoAuthorization(path string, method string, headers map[string]string) (bool, int, string) {
	...
	//Tenemos que autorizar

	token := headers["authoriazation"]
	//Aqui corroboro si tengo un token o no, si es == a cero no tengo nada
	if len(token) == 0 {
